                    ...
                    {batchResult && (
                        <div className="mt-4 p-4 bg-gray-50 border rounded-lg">
                            ...
                            {batchResult.details.filter(d => d.status === 'error').length > 0 && (
                                <div className="max-h-32 overflow-y-auto border-t border-gray-200 pt-2 mb-2 mt-2">
                            ...
                            {batchResult.details.filter(d => d.status === 'success' && d.url).length > 0 && (
                                <div className="max-h-48 overflow-y-auto border-t border-gray-200 pt-2 mt-2">
                                    <p className="text-xs font-medium text-green-600 mb-2">ลิงก์คำเชิญ (สามารถคัดลอกได้ที่นี่):</p>
                                    <ul className="space-y-2">
                                        {batchResult.details.filter(d => d.status === 'success' && d.url).map((d, i) => (
                                            <li key={i} className="text-xs bg-white p-2 border border-gray-200 rounded-md">
                                                <div className="font-medium text-blue-700 mb-1">{d.email}</div>
                                                <div className="flex gap-2">
                                                    <input readOnly value={d.url} className="text-xs border border-gray-300 rounded px-2 py-1 flex-1 bg-gray-50 text-gray-500" />
                                                    <button type="button" onClick={() => navigator.clipboard.writeText(d.url)} className="p-1 px-2 text-blue-700 bg-blue-100 hover:bg-blue-200 rounded border border-blue-200 transition-colors flex items-center">
