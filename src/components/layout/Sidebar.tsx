...
function SidebarContent({ isOpen, onToggle }: SidebarProps) {
  ...
  useEffect(() => {
    if (!user?.id || !user?.sites || user.sites.length === 0) {
      setSites([]);
      return;
    }

    const safeSites = user.sites.slice(0, 30);
    const loadedSites = new Map<string, Site>();
    const unsubscribes: (() => void)[] = [];

    safeSites.forEach(siteId => {
      const unsub = onSnapshot(doc(db, "sites", siteId), (docSnap) => {
        if (docSnap.exists()) {
          const data = docSnap.data();
          loadedSites.set(siteId, {
            id: docSnap.id,
            name: data.name,
            userOverrides: data.userOverrides || {}
          });
          // สร้าง array ใหม่เพื่อให้ React มองเห็นการเปลี่ยนแปลง
          setSites(Array.from(loadedSites.values()));
        } else {
          // ถ้าลบไปแล้ว ก็เอาออก
          loadedSites.delete(siteId);
          setSites(Array.from(loadedSites.values()));
        }
      }, (error) => {
        console.warn(`Sidebar: Failed to fetch site ${siteId}`, error);
      });
