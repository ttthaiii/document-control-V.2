...
import UserSiteModal from '@/components/admin/UserSiteModal'; // 👈 Import Modal ใหม่
...
export default function ManageUsersPage() {
  ...
  const [selectedUserForSites, setSelectedUserForSites] = useState<UserData | null>(null);

  // สกัดฟังก์ชัน fetchUsers ออกมาเพื่อเรียกใช้ซ้ำ
  ...
  return (
    <AuthGuard requiredRoles={[ROLES.ADMIN]}>
      <Layout>
        <div className="max-w-7xl mx-auto">
          <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input type="text" placeholder="ค้นหาชื่อ, รหัสพนักงาน, อีเมล..." className="w-full h-10 pl-10 pr-4 border rounded-lg focus:ring-2 focus:ring-blue-500 bg-white text-gray-900 placeholder-gray-500 outline-none"
                      ...
                      {filteredUsers.map((user) => (
                        <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                            <button
                              onClick={() => setSelectedUserForSites(user)}
                              className="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                            >
                              <AlertCircle size={14} className="mr-2 text-blue-500" /> จัดการโครงการ
                            </button>
                            <button
                              onClick={() => setSelectedUserForPerms(user)}
                              className="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                            >
                              <Sliders size={14} className="mr-2 text-gray-500" /> ตั้งค่าสิทธิ์
                            </button>
                          </td>
                        </tr>
                      ))}
        ...
        {selectedUserForPerms && <UserPermissionModal user={selectedUserForPerms} onClose={() => setSelectedUserForPerms(null)} />}
        {selectedUserForSites && <UserSiteModal user={selectedUserForSites} onClose={() => setSelectedUserForSites(null)} onSave={fetchUsers} />}
      </Layout>
    </AuthGuard>
  );
}